...
fn bench_pread_ctx_vec(b: &mut test::Bencher) {
    ...
    b.iter(|| {
        ...
        for val in data.chunks(2) {
            let _: Result<u16, _> = black_box(val.pread_into(0));
        }
    });
...
fn bench_pread_unwrap(b: &mut test::Bencher) {
    ...
    b.iter(|| {
        for _ in 1..NITER {
            ...
            let _: u16 = black_box(data.pread(0, LE).unwrap());
        }
    });
...
fn bench_pread_vec(b: &mut test::Bencher) {
    ...
    b.iter(|| {
        let data = black_box(&vec[..]);
        for val in data.chunks(2) {
            let _: Result<u16, _> = black_box(val.pread(0, LE));
        }
    });
    b.bytes = vec.len() as u64;
}
...
#[bench]
fn bench_pread_into_unwrap(b: &mut test::Bencher) {
    use scroll::{Pread};
    const NITER: i32 = 100_000;
    b.iter(|| {
        for _ in 1..NITER {
            let data = black_box([1, 2]);
            let _: u16 = black_box(data.pread_into(0)).unwrap();
        }
    });
    b.bytes = 2 * NITER as u64;
}
...
fn bench_gread_vec(b: &mut test::Bencher) {
    ...
    b.iter(|| {
        ...
        for val in data.chunks(2) {
            let mut offset = 0;
            let _: Result<u16, _> = black_box(val.gread_into(&mut offset));
        }
    });
    b.bytes = vec.len() as u64;
}
...
#[bench]
fn bench_parallel_pread(b: &mut test::Bencher) {
    use scroll::{Pread, LE};
    use rayon::prelude::*;
    let vec = vec![0u8; 1_000_000];
    ...
    b.iter(|| {
        ...
        nums.par_iter().for_each(| offset | {
            let _: Result<u16, _> = black_box(data.pread(*offset, LE));
        });
    });
    b.bytes = vec.len() as u64;
}
